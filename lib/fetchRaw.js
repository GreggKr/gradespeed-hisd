...
const baseURL = 'https://publicapps.houstonisd.org/ParentStudentConnect'
const bodyArr = []
// let body = null;
let cookie = null
let cnt = 0
...
const fetchRaw = (username, password) => (
  new Promise((resolve, reject) => {
    return got(baseURL + '/', {
      ...
      .catch(response => {
        if (response.url === 'https://publicapps.houstonisd.org/ParentStudentConnect/login.aspx' && response.statusCode === 302) {
          cookie += ' ' + response.headers['set-cookie'][0].split(' ')[0]
            ...
            .then(response => {
              ...
              const getBody = response => {
                ...
                if (cnt === gradeLinkArr.length) {
                  fs.writeFile("output.txt", bodyArr, (err) => {
                    if (err) console.error(err);
                    console.log("Completed. Output file: output.txt");
                  });
                }
              }
